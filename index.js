...
function addMessage(name, message, date) {
	...
	const messageContext = generateElement('p', 'message-text','',message);
	messageContext.appendChild(generateElement(
		'button',
		'btn-remove',
		'',
		'remove'
	...
	newMessage.addEventListener('click',e=>{
		if(e.target == e.target.closest('.btn-remove')){
			newMessage.remove();
			if(messagesContainer.childNodes.length <= 3 ){//странная проверка получилась, но метод .hasChildNodes() постоянно показывал true
				notEmpty = false;
